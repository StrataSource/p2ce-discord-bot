...
export function error(client: Client, msg: Error): Promise<Message<boolean>> | undefined {
	...
	return message(client, 'ERROR', LogLevelColor.ERROR, `${msg.name}: ${msg.message}`);
}

export function warning(client: Client, msg: string): Promise<Message<boolean>> | undefined {
	console.log(`[WARNING] ${message}`);
	return message(client, 'WARNING', LogLevelColor.WARNING, msg);
}

export function userUpdate(client: Client, user1: User | PartialUser, user2: User): Promise<Message<boolean>> | undefined {
	if (user1.username !== user2.username) {
		return message(client, 'USER', LogLevelColor.INFO, `**${user1.username}#${user1.discriminator}** changed their username to **${user2.username}#${user2.discriminator}**`);
	} else if (user1.avatar !== user2.avatar) {
		return message(client, 'USER', LogLevelColor.INFO, `**${user1.username}#${user1.discriminator}** changed their avatar`);
	} else {
		return undefined;
	}
}
...
export function userBanned(client: Client, ban: GuildBan): Promise<Message<boolean>> | undefined {
	return message(client, 'BAN', LogLevelColor.IMPORTANT, `**${ban.user.username}#${ban.user.discriminator}** was banned`, ban.user.avatarURL());
}

export function messageDeleted(client: Client, msg: Message<boolean> | PartialMessage): Promise<Message<boolean>> | undefined {
	if (msg.author?.bot || msg.author?.username === undefined) return undefined;
	if (msg.cleanContent) {
		return message(client, 'MESSAGE', LogLevelColor.INFO, `A message from **${msg.author?.username}#${msg.author?.discriminator}** was deleted in ${msg.channel.toString()}\n\nContents: \`\`\`${msg.cleanContent}\`\`\``, msg.author?.avatarURL());
	} else {
...
export function messageUpdated(client: Client, oldMessage: Message<boolean> | PartialMessage, newMessage: Message<boolean> | PartialMessage): Promise<Message<boolean>> | undefined {
	...
	if (oldMessage.cleanContent !== newMessage.cleanContent) {
		return message(client, 'MESSAGE', LogLevelColor.INFO, `A message from **${oldMessage.author?.username}#${oldMessage.author?.discriminator}** was edited in ${oldMessage.channel.toString()}\n\nBefore: \`\`\`${oldMessage.cleanContent}\`\`\`\nAfter: \`\`\`${newMessage.cleanContent}\`\`\``, oldMessage.author?.avatarURL());
	} else {
		return undefined;
	}
}
